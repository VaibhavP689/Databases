...
def method1():
    ...
    df = pd.read_excel("Project_1.xlsx", sheet_name="Sheet4", index_col=0)

    # Check if the database file exists and remove it if it does
    if os.path.exists('mydb4.db'):
        try:
            os.remove('mydb4.db')
    ...
    engine = create_engine('sqlite:///mydb4.db')
    conn: Engine.connect = engine.connect()

    # Write records stored in a DataFrame to a SQL database
    df.to_sql("Sheet4Table", con=engine, if_exists='replace', index=False)
    ...
    conn = sqlite3.connect('mydb4.db')
    cursor = conn.cursor()

    # Execute a query to fetch data
    cursor.execute('SELECT * FROM Sheet4Table')
