...
def method1():
    ...
    df = pd.read_excel("Project_1.xlsx", sheet_name="Sheet2", index_col=0)

    # Check if the database file exists and remove it if it does
    if os.path.exists('mydb2.db'):
        try:
            os.remove('mydb2.db')
    ...
    df.to_sql("Sheet2Table", con=engine, if_exists='replace', index=False)
    ...
    cursor.execute('SELECT * FROM Sheet2Table')
