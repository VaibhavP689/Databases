document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('deleteColumnBtn').addEventListener('click', () => {
        ...
        // console.log(gridApi.getFocusedCell());
        // console.log(gridApi.getAllDisplayedColumns().indexOf(gridApi.getFocusedCell().column));
    });
    ...
    function exportGridDataToJson(gridApi, oldLength) {
        ...
        const allRowData = [...rowData, ...hiddenRows]; // Include hidden rows

        // Restore original order
        allRowData.sort((a, b) => a.originalIndex - b.originalIndex);

        const filteredRowData = allRowData.filter(obj => !isAllValuesNull(obj));
        const newLength = filteredRowData.length;
        let diff = newLength - oldLength;
        const json = JSON.stringify(filteredRowData.map(row => {
            delete row.originalIndex; // Remove the originalIndex before saving
            return row;
        }), null, 2);
    ...
    function hideSelectedRows(gridApi) {
        ...
        hiddenRows.push(...selectedData); // Add to hiddenRows array
